<%@ page language="java" contentType="text/html; charset=utf-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>Insert title here</title>
<link rel="stylesheet"
	href="${pageContext.request.contextPath}/resources/css/homepage2.css"
	type="text/css">
	
</head>
 <style>
 
</style> 
<body>

	<div class="header">
		<div class="header_in">
			<div id = "logo">
			<a href="MainHome.jsp">
				<img src="${pageContext.request.contextPath}/resources/images/logo.png"/>
			</a>
			</div>
			
			

		
				<div class ="menu_list" id="test">
				<ul id = "menu_main" >
					<li><a href ="./Event.jsp">이벤트</a></li>
					<li><a href ="#">리스트2</a></li>
					<li><a href ="#">리스트3</a></li>
					<li><a href ="#">리스트4</a></li>
					<li><a href ="#">리스트5</a></li>
					<li><a href ="#">리스트6</a></li>
				</ul>
				
		<!-- 	 	<ul id = "menu_sub">
					<li>공지사항</li>
					<li><a href="https://www.facebook.com/GongCha.Korea"> <img
							src="resources/images/facebook.png"></a> <a
						href="https://www.instagram.com/gongcha.official/"> <img
							src="resources/images/instargram.png">
					</a></li>
					<li id = "store_logo" >
					<a href="#"></a>
						<img src="resources/images/sub_menu_logo1.png">
						<div>가맹점개설문의010-1111-1111</div>
					</li>
				</ul> -->
	<div id="hd_name"><!--슬라이드바  -->
	</div>
	
	<div id="imgs">
		<a href = "#">
			<img src ="${pageContext.request.contextPath}/resources/images/eventpicture.jpg"/>
		</a>
	</div>
	
	<div id = "hd_submenu">
	
		<table>
		<tr>
			<td>
			<a href="#">브랜드 소개</a>
			</td>
				<td>
					<a href="#">브랜드 소개</a>
			</td>
			<td>
					<a href="#">브랜드 소개</a>
			</td>
			<td>
					<a href="#">브랜드 소개</a>
			</td>
			<td>
					<a href="#">브랜드 소개</a>
			</td>
			<td>
					<a href="#">브랜드 소개</a>
			</td>
		</tr>
		
		
			<tr>
			<td>
			<a href="#">브랜드 소개</a>
			</td>
				<td>
					<a href="#">브랜드 소개</a>
			</td>
			<td>
					<a href="#">브랜드 소개</a>
			</td>
			<td>
					<a href="#">브랜드 소개</a>
			</td>
			<td>
					<a href="#">브랜드 소개</a>
			</td>
			<td>
					<a href="#">브랜드 소개</a>
			</td>
		</tr>
			
			
			<tr>
			<td>
			<a href="#">브랜드 소개</a>
			</td>
				<td>
					<a href="#">브랜드 소개</a>
			</td>
			<td>
					<a href="#">브랜드 소개</a>
			</td>
			<td>
					<a href="#">브랜드 소개</a>
			</td>
			<td>
					<a href="#">브랜드 소개</a>
			</td>
			<td>
					<a href="#">브랜드 소개</a>
			</td>
		</tr>
		
		
			<tr>
			<td>
			<a href="#">브랜드 소개</a>
			</td>
				<td>
					<a href="#">브랜드 소개</a>
			</td>
			<td>
					<a href="#">브랜드 소개</a>
			</td>
			<td>
					<a href="#">브랜드 소개</a>
			</td>
			<td>
					<a href="#">브랜드 소개</a>
			</td>
			<td>
					<a href="#">브랜드 소개</a>
			</td>
		</tr>
		</table>
	
	
	</div> 
</div>
</div>
</div>
		

</body>
</html>